 ...
 */
package org.obiba.opal.core.utils;
...
import org.obiba.opal.core.git.GitNote;
import org.obiba.opal.core.git.PathModel;
import org.obiba.opal.core.git.RefModel;
import org.obiba.opal.core.git.SearchType;
import org.obiba.opal.core.git.SubmoduleModel;
...
public class JGitUtils {
  ...
  public static String getDisplayName(PersonIdent person) {
    if(Strings.isNullOrEmpty(person.getEmailAddress())) {
   ...
   */
  public static List<String> getRepositoryList(File repositoriesFolder, boolean onlyBare, boolean searchSubfolders,
      int depth, List<String> exclusions) {
    ...
    list.addAll(
        getRepositoryList(repositoriesFolder.getAbsolutePath(), repositoriesFolder, onlyBare, searchSubfolders, depth,
            patterns));
  ...
  /**
   * Recursive function to find utils repositories.
   *
  ...
  public static List<PathModel.PathChangeModel> getFilesInCommit(Repository repository, RevCommit commit) {
    List<PathModel.PathChangeModel> list = new ArrayList<PathModel.PathChangeModel>();
    if(!hasCommits(repository)) {
      return list;
    }
    RevWalk rw = new RevWalk(repository);
    try {
      ...
      if(commit.getParentCount() == 0) {
        ...
        tw.addTree(commit.getTree());
        while(tw.next()) {
          list.add(new PathModel.PathChangeModel(tw.getPathString(), tw.getPathString(), 0, tw.getRawMode(0),
        ...
        for(DiffEntry diff : diffs) {
          ...
          if(diff.getChangeType().equals(ChangeType.DELETE)) {
            list.add(new PathModel.PathChangeModel(diff.getOldPath(), diff.getOldPath(), 0, diff.getNewMode().getBits(), objectId,
                commit.getId().getName(), diff.getChangeType()));
          } else if(diff.getChangeType().equals(ChangeType.RENAME)) {
            list.add(new PathModel.PathChangeModel(diff.getOldPath(), diff.getNewPath(), 0, diff.getNewMode().getBits(), objectId,
                commit.getId().getName(), diff.getChangeType()));
          } else {
            list.add(new PathModel.PathChangeModel(diff.getNewPath(), diff.getNewPath(), 0, diff.getNewMode().getBits(), objectId,
                commit.getId().getName(), diff.getChangeType()));
          }
        }
      }
   ...
   */
  public static List<PathModel.PathChangeModel> getFilesInRange(Repository repository, RevCommit startCommit,
      RevCommit endCommit) {
    List<PathModel.PathChangeModel> list = new ArrayList<PathModel.PathChangeModel>();
    if(!hasCommits(repository)) {
      return list;
    ...
    try {
      ...
      for(DiffEntry diff : diffEntries) {
        ...
        if(diff.getChangeType().equals(ChangeType.DELETE)) {
          list.add(new PathModel.PathChangeModel(diff.getOldPath(), diff.getOldPath(), 0, diff.getNewMode().getBits(),
              diff.getOldId().name(), null, diff.getChangeType()));
        } else if(diff.getChangeType().equals(ChangeType.RENAME)) {
          list.add(new PathModel.PathChangeModel(diff.getOldPath(), diff.getNewPath(), 0, diff.getNewMode().getBits(),
              diff.getNewId().name(), null, diff.getChangeType()));
        } else {
          list.add(new PathModel.PathChangeModel(diff.getNewPath(), diff.getNewPath(), 0, diff.getNewMode().getBits(),
              diff.getNewId().name(), null, diff.getChangeType()));
        }
      }
      Collections.sort(list);
  ...
  public static List<RevCommit> getRevLog(Repository repository, String objectId, String path, int offset,
    ...
    try {
      ...
      if(!StringUtils.isEmpty(path)) {
        TreeFilter filter = AndTreeFilter
            .create(PathFilterGroup.createFromStrings(Collections.singleton(path)), TreeFilter.ANY_DIFF);
      ...
      if(!target.startsWith(Constants.R_HEADS)) {
        ...
        if(tagModels.size() > 0) {
          ...
          for(RefModel tagModel : tagModels) {
            if(tagModel.getReferencedObjectId().getName().equals(target) && tagModel.getDate().after(lastDate)) {
              tag = tagModel;
      ...
      switch(result) {
        ...
        default:
          LOGGER.error(MessageFormat
              .format("{0} HEAD update to {1} returned result {2}", repository.getDirectory().getAbsolutePath(),
                  targetRef, result));
      }
    } catch(Throwable t) {
  ...
  public static boolean setBranchRef(Repository repository, String branch, String commitId) {
    ...
    try {
      ...
      switch(result) {
        ...
        default:
          LOGGER.error(MessageFormat
              .format("{0} {1} update to {2} returned result {3}", repository.getDirectory().getAbsolutePath(),
  ...
  public static boolean deleteBranchRef(Repository repository, String branch) {
    ...
    try {
      ...
      switch(result) {
        ...
        default:
          LOGGER.error(MessageFormat
              .format("{0} failed to delete to {1} returned result {2}", repository.getDirectory().getAbsolutePath(),
                  branchName, result));
      }
    } catch(Throwable t) {
  ...
  public static boolean createIncrementalRevisionTag(Repository repository, String objectId, PersonIdent tagger,
    ...
    Iterator<Map.Entry<String, Ref>> iterator = repository.getTags().entrySet().iterator();
    long lastRev = 0;
