...
class RValueSetFetcher {
  ...
  RServerResult getResult(VariableEntity entity) {
    ...
    String cmd = String.format("haven::zap_labels(tibble::as_tibble(`%s` %%>%% filter(`%s` == %s)))", table.getSymbol(), table.getIdColumn(), rid);
  ...
  RServerResult getResult(List<VariableEntity> entities) {
    ...
    String cmd = String.format("haven::zap_labels(tibble::as_tibble(`%s` %%>%% filter(`%s` %%in%% c(%s))))", table.getSymbol(), table.getIdColumn(), ids);
    return table.execute(cmd);
  }
