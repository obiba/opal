export default {
  ...
  r_packages_warn: 'Cette opération peut paraître pratique mais elle n\'est pas recommandée dans un environnement de production. Il est préférable de déployer le serveur R à partir d\'une image Docker validée pour garantir la reproductibilité de l\'environnement d\'analyse R.',
  ...
  datashield_packages_info: 'Les packages DataSHIELD sont des packages R qui fournissent des fonctionnalités DataSHIELD. Ils sont installés sur les serveurs R et sont utilisés sont utiliséspar les clients DataSHIELD pour effectuer des analyses.',
  datashield_profiles_info: 'Un profile DataSHIELD est associé à un cluster de serveurs R: les méthodes et options extraits des packages DataSHIELD installés définissent la configuration du profile. Celle-ci peut être modifiée par la suite. ',
  add_profile: 'Ajouter un profile',
  datashield_profile_missing_cluster: 'Le cluster de serveurs R pour ce profile DataSHIELD n\'existe pas.',
  datashield_profile_status_toggle: 'Activer or désactivater le profile',
  settings: 'Configuration',
  datashield_access_not_restricted: 'Tous les utilisateurs DataSHIELD peuvent accèder à ce profile.',
  datashield_access_restricted: 'Seuls les utilisateurs DataSHIELD autorisés peuvent accèder à ce profile.',
  datashield_settings_init: 'Initialiser la configuration DataSHIELD du profile à partir des packages DataSHIELD sélectionnés',
  datashield_settings_init_help: 'Selectionner quels packages DataSHIELD auront leurs méthodes et options appliqués à ce profile.',
  type: 'Type',
  r_script: 'Script R',
  r_func: 'Fonction R',
  code: 'Code',
  ...
  delete_datashield_methods_confirm: '- | Êtes-vous sûr de vouloir supprimer cette méthode DataSHIELD ? | Êtes-vous sûr de vouloir supprimer ces {count} méthodes DataSHIELD ?',
  delete_datashield_options_confirm: ' - | Êtes-vous sûr de vouloir supprimer cette option DataSHIELD ? | Êtes-vous sûr de vouloir supprimer ces {count} options DataSHIELD ?',
  datashield_option_name_hint: 'Nom de l\'option R.',
  datashield_option_value_hint: 'Valeur de l\'option R. Une syntaxe R est attendue : par exemple utiliser des guillemets pour les chaînes de caractère.',
  datashield_method_name_hint: 'Le nom de la méthode DataSHIELD doit être unique. C\'est le nom de la fonction autorisée pouvant être appelée par un client DataSHIELD.',
  datashield_method_func_hint: 'La fonction R étant effectivement appellée côté serveur. Assurez vous qu\'il n\'y a pas d\'ambiguité en spécifiant le package R package dans lequel elle est définie.',
  datashield_method_script_hint: 'Le code de la fonction R adhoc qui sera appelée.',
  function: 'Fonction',
  datashield_aggregate_methods_info: 'Les méthodes d\'agrégation sont utilisées par DataSHIELD afin de compiler des données individuelles. Les mêmes méthodes d\'agrégation doivent être définies dans chaque serveur DataSHIELD qui sera impliqué dans un processus de calcul. Chaque méthode d\'agrégation est identifiée par un nom qui sera utilisé depuis le client DataSHIELD.',
  datashield_assign_methods_info: 'Les méthodes d\'affectation sont utilisées par DataSHIELD afin de transformer des données individuelles côté serveur. Les mêmes méthodes d\'affectation doivent être définies dans chaque serveur DataSHIELD qui sera impliqué dans un processus de calcul. Chaque méthode d\'affectation est identifiée par un nom qui sera utilisé depuis le client DataSHIELD.',
  datashield_options_info: 'Les options R sont appliquées lors de la création de la session DataSHIELD et affectent le comportement de certaines méthodes DataSHIELD.',
};
