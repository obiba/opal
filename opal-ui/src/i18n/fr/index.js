export default {
  ...
  db: {
    ...
    owner_project: 'Projet',
    owner_project_hint: 'Cette base de données appartient à un projet et est gérée par Opal.',
    delete_files: 'Supprimer également les fichiers de la base de données',
    delete_files_hint: "La base de données H2 est un fichier du dossier de données d'Opal. S'il est conservé, il garde les identifiants de connexion avec lesquels il a été créé, et une base de données enregistrée à nouveau sur la même URL avec un mot de passe différent ne pourra pas l'ouvrir.",
    delete_internal_confirm: "Êtes-vous sûr de vouloir supprimer la base de données '{name}' ? Notez que les données et ses fichiers SERONT définitivement supprimés.",
    unregister: 'Retirer',
  ...
  kubernetes: {
    ...
    rock_info: "Rock est un serveur R avec une API REST (voir projet {url}). Les pods Rock créés à la demande offrent une capacité d'analyse R évolutive et sécurisée.",
    add_pod_spec: 'Ajouter une spécification de pod',
    edit_pod_spec: 'Modifier la spécification de pod',
    id_hint: "L'identifiant de la spécification doit être unique. C'est le nom qui sera utilisé pour identifier le profile R.",
    ...
    environment_hint: "Les variables d'environnement à définir dans le pod. Ces variables peuvent être utilisées pour configurer le comportement du pod.",
    image: 'Image',
    image_hint: "L'image Docker à utiliser pour créer le pod. L'image doit être accessible depuis le cluster Kubernetes.",
    image_pull_policy: "Politique de tirage d'image",
    image_pull_policy_hint: "La politique de tirage d'image à utiliser lors de la création du pod.",
    image_pull_secret: "Secret de tirage d'image",
    image_pull_secret_hint: "Le secret Kubernetes à utiliser pour tirer l'image Docker depuis un registre privé.",
    resources_requests: 'Demandes de ressources',
    resources_requests_hint: "Les ressources demandées par le pod. Ces ressources sont réservées au pod et ne peuvent pas être utilisées par d'autres pods.",
    resources_limits: 'Limites de ressources',
    ...
    node: 'Nœud',
    node_hint: 'Le nœud Kubernetes sur lequel le pod sera créé. Si laissé vide, le planificateur Kubernetes choisira un nœud approprié.',
    node_name: 'Nom du nœud',
    node_name_hint: "Les différentes stratégies pour l'assignation du pod à un nœud Kubernetes. Si laissé vide, le planificateur Kubernetes choisira un nœud approprié.",
    node_selector: 'Sélecteur de nœud',
  ...
  user_profile: {
    ...
    load_error: "Erreur lors du chargement du profil utilisateur '{user}'",
  ...
  validation: {
    r_quota: {
      subject_required: "Un nom de groupe ou d'utilisateur est requis",
      limit_required: 'Une limite de zéro minute ou plus est requise',
    },
  ...
  r_quota: {
    title: 'Quotas',
    info: "Un quota limite ce qu'un utilisateur peut consacrer à DataSHIELD sur une fenêtre glissante, en temps d'exécution R ou en temps de session R. La fenêtre glisse : il n'y a pas de remise à zéro, la capacité revient à mesure que l'activité passée sort de la fenêtre. Un utilisateur se voit appliquer son propre quota s'il en a un, sinon le plus permissif des quotas des groupes auxquels il appartient, sinon le quota par défaut du système. Les deux métriques sont résolues séparément, un sujet peut donc recevoir un quota de chaque, et si aucun ne s'applique l'usage est illimité.",
    add: 'Ajouter un quota',
    edit: 'Modifier le quota',
    user: 'Utilisateur',
    group: 'Groupe',
    delete_confirm: "Êtes-vous sûr de vouloir supprimer le quota de {subject} ? Son sujet redevient illimité, à moins qu'un quota plus large ne s'applique encore à lui.",
    subject: 'Sujet',
    subject_type: "S'applique à",
    subject_type_hint: "Un quota s'applique à tout le monde (défaut du système), aux membres d'un groupe, ou à un seul utilisateur.",
    system_default: 'Défaut du système',
    group_hint: "Le nom du groupe auquel s'applique le quota. Un utilisateur appartenant à plusieurs groupes obtient le plus permissif de leurs quotas.",
    user_hint: "Le nom de l'utilisateur auquel s'applique le quota. Un quota personnel prime sur tout quota de groupe et sur le défaut du système.",
    metric: 'Métrique',
    metric_hint: "Ce sur quoi l'allocation est dépensée. Le temps d'exécution facture le cpu du serveur R consommé et compte une session inactive pour rien ; le temps de session facture le serveur R que l'utilisateur mobilise, qu'il calcule ou non.",
    metric_execution_time: "Temps d'exécution",
    metric_session_time: 'Temps de session',
    period: 'Fenêtre',
    period_hint: "La fenêtre glissante sur laquelle l'usage est cumulé.",
    period_daily: 'Dernières 24 heures',
    period_weekly: 'Derniers 7 jours',
    limit: 'Limite',
    limit_minutes: 'Limite (minutes)',
    limit_minutes_hint: "Temps d'exécution R autorisé sur la fenêtre. Zéro interdit complètement DataSHIELD à ce sujet.",
    limit_minutes_hint_session_time: "Durée pendant laquelle les sessions R peuvent rester ouvertes sur la fenêtre, temps d'inactivité compris. Zéro interdit complètement DataSHIELD à ce sujet.",
    limit_below_execution_time: "Cette limite de temps de session est inférieure à la limite de temps d'exécution du même sujet. Une session dure au moins aussi longtemps que les commandes qu'elle exécute : la limite de temps d'exécution ne pourra jamais être atteinte.",
    used: 'Consommé',
    usage_title: 'Quota {context}',
    usage_used: '{used} sur {limit} de {metric} consommés — {window}',
    usage_from_user: "Provient d'un quota personnel.",
    usage_from_group: 'Provient du quota du groupe "{principal}".',
    usage_from_system: 'Provient du quota par défaut du système.',
    usage_unlimited: "Aucun quota ne s'applique : le {metric} {context} est illimité.",
    usage_unavailable: "La consommation du quota n'est pas disponible.",
    usage_blocked: 'Les nouvelles sessions {context} sont bloquées. Les sessions déjà ouvertes continuent de fonctionner.',
    usage_credit: 'De la capacité revient {distance}.',
    usage_open_sessions: '· {count} session ouverte, qui consomme toujours | · {count} sessions ouvertes, qui consomment toujours',
    usage_close_sessions: "Attendre ne sert à rien : fermez la session que vous n'utilisez pas. | Attendre ne sert à rien : fermez les sessions que vous n'utilisez pas.",
    set_personal: 'Définir un quota personnel',
    edit_personal: 'Modifier ce quota',
    enabled_hint: "Un quota désactivé ne s'applique pas, et n'exempte pas non plus son sujet : c'est le quota d'un groupe, ou le défaut du système, qui s'applique alors.",
  },
  resource_ref: {
  ...
  extract: 'Extraire',
  failed: 'Échoué',
  file_bundle_cancelled: 'Le téléchargement de fichiers a été annulé.',
  file_bundle_failed: 'La préparation du téléchargement de fichiers a échoué.',
  ...
  install: 'Installer',
  internal_database: '(Base de données interne)',
  internal_database_hint: 'Opal crée une base de données pour ce projet et la supprime avec le projet.',
  ...
  refresh: 'Rafraîchir',
  registered_database_hint: 'Les données sont conservées lorsque le projet est supprimé.',
