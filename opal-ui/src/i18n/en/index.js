export default {
  ...
  identity_providers_info: 'Users from a declared <a href="https://en.wikipedia.org/wiki/Identity_provider" target="_blank">Identity Provider</a> can sign in the application using their credentials. Once they have signed in, the users are automatically associated to a group having the Identity Provider\'s name, so that permissions can be applied to all the users from the same source. Additional group associations can be applied too. Note that only Identity Providers of type <a href="https://en.wikipedia.org/wiki/OpenID_Connect" target="_blank">OpenID Connect</a> are supported.',
