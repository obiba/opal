export default {
  ...
  r: 'R',
  r_servers: 'R servers',
  r_servers_info: 'R servers are grouped by clusters. In each cluster all the R servers are considered to be interchangeable, i.e. a load balancer can select one or the other on R session creation.',
  r_cluster: 'Cluster',
  r_clusters_count: 'No cluster of R servers | 1 cluster of R servers | {count} clusters of R servers',
  r_version: 'R version',
  r_system: '{cores} cores, {memory} free memory',
  r_sessions: 'R sessions',
  r_sessions_info: 'R server activity monitoring: each R session is a computation unit started by R/DataSHIELD users or by internal processes. R sessions without activity for a while will be automatically terminated.',
  r_workspaces: 'R workspaces',
  r_workspaces_info: 'R workspaces storage: each saved R/DataSHIELD workspace contains the session\'s image and files (if any). These can be restored any number of times on user demand.',
  servers: 'Servers',
  server: 'Server',
  packages: 'Packages',
  ...
  r_packages_warn: 'This operation may seem convenient but it is not recommended in a production environment. It is best to deploy the R server using a validated Docker image to ensure reproducibility of the R analysis environment.',
  ...
  datashield_profiles_info: 'A DataSHIELD profile is associated to a R servers cluster: the methods and options are extracted from the installed DataSHIELD packages settings and can be amended. ',
  add_profile: 'Add profile',
  datashield_profile_missing_cluster: 'The R servers cluster for this DataSHIELD profile does not exist.',
  datashield_profile_status_toggle: 'Activate or deactivate profile',
  settings: 'Settings',
  datashield_access_not_restricted: 'Any DataSHIELD user can access this profile.',
  datashield_access_restricted: 'Only permitted DataSHIELD users can access this profile.',
  datashield_settings_init: 'Initialize the DataSHIELD profile settings with selected DataSHIELD packages',
  datashield_settings_init_help: 'Select which DataSHIELD packages will get their methods and options applied to this profile.',
  type: 'Type',
  r_script: 'R script',
  r_func: 'R function',
  code: 'Code',
  ...
  delete_datashield_methods_confirm: '- | Are you sure you want to delete this DataSHIELD method? | Are you sure you want to delete these {count} DataSHIELD methods?',
  delete_datashield_options_confirm: '- | Are you sure you want to delete this R option? | Are you sure you want to delete these {count} R options?',
  datashield_option_name_hint: 'R option name.',
  datashield_option_value_hint: 'R option value. R syntax is expected: for instance use quotes for string values.',
  datashield_method_name_hint: 'The name of the DataSHIELD method name must be unique. This is the name of the allowed function that can be called by the DataSHIELD client.',
  datashield_method_func_hint: 'The R server side function that will be effectively called. Make sure there is no ambiguity by specifying the R package in which it is defined.',
  datashield_method_script_hint: 'A custom R function implementation that will be effectively called.',
  function: 'Function',
  datashield_aggregate_methods_info: 'The aggregation methods are used by DataSHIELD in order to compile individual data. The same aggregation methods must be defined in each DataSHIELD server that will be involved in a computation process. Each aggregation method is identified by a name that will be used from the DataSHIELD client.',
  datashield_assign_methods_info: 'The assign methods are used by DataSHIELD in order to transform individual data on server side. The same assign methods must be defined in each DataSHIELD server that will be involved in a computation process. Each assign method is identified by a name that will be used from the DataSHIELD client.',
  datashield_options_info: 'The R options are applied at DataSHIELD R session creation and affect the behavior of some of the DataSHIELD methods.',
};
