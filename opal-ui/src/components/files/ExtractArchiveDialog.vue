...
function onSubmit() {
  ...
  filesStore.extractArchive(props.file.path, destinationFolder.value, password.value === '' ? undefined : password.value).then(() => {
    if (destinationFolder.value)
      filesStore.loadFiles(destinationFolder.value.path);
  });
}
