    ...
    <q-card class="dialog-sm">
      ...
      <q-card-actions align="right" class="bg-grey-3">
        ...
        <q-btn
          ...
          :disable="name.length === 0 || (type.value === 'r_func' && func.length === 0) || (type.value === 'r_script' && script.length === 0)"
          color="primary"
          @click="onSubmit"
          v-close-popup
        />
...
function getCode(method: DataShieldMethodDto): string {
  if (getType(method).value === 'r_func') {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const funcMethod = (method as any)['DataShield.RFunctionDataShieldMethodDto.method'] as RFunctionDataShieldMethodDto;
