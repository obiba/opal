...
watch(() => props.modelValue, (value) => {
  if (value) {
    ...
    if (props.view) {
      datasourceStore.getAllTables(props.table.entityType)
      .then((tables) => allTables.value = tables
...
const isTableNameValid = computed(() => datasourceStore.isNewTableNameValid(name.value) || props.table.name === name.value);
...
function onSaveTable() {
  if (props.view && isView.value) {
    ...
    datasourceStore.updateTable(props.table, updatedTable)
