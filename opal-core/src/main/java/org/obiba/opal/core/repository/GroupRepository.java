...
public interface GroupRepository extends JpaRepository<Group, Long> {
  ...
  /**
   * Delete the stored Group identified by its natural key, whether or not the object handed in is the one that was
   ...
   */
  default void deleteByKey(Group group) {
    findByName(group.getName()).ifPresent(this::delete);
  }

  default Group upsert(Group group) {
    return save(EntityKeys.reuseKey(findByName(group.getName()).orElse(null), group));
  }
}
