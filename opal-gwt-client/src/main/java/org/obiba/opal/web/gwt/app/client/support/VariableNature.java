...
public enum VariableNature {
  ...
  public static VariableNature getNature(VariableDto variable) {
    if(variable.getCategoriesArray().length() > 0) {
      ...
      return allMissing ? CONTINUOUS : CATEGORICAL;
    }

    if("integer".equals(variable.getValueType()) || "decimal".equals(variable.getValueType())) {
      return CONTINUOUS;
    } else if("date".equals(variable.getValueType()) || "datetime".equals(variable.getValueType())) {
      return TEMPORAL;
    } else if("boolean".equals(variable.getValueType())) {
      return CATEGORICAL;
    } else if("linestring".equals(variable.getValueType()) || "point".equals(variable.getValueType()) ||
        "polygon".equals(variable.getValueType())) {
      return GEO;
    } else {
      return UNDETERMINED;
    }
