...
import org.obiba.opal.web.gwt.app.client.ui.TaxonomiesModalPanel;
...
//        panel.clear();
  ...
  void onSaveTaxonomy(ClickEvent event) {
    ...
    for(int i = 0; i < availableLocales.size(); i++) {
      if(!panelList.get(i).getTaxonomyTitle().isEmpty()) {
        titles.push(asTextDto(panelList.get(i).getTaxonomyTitle(), availableLocales.get(i)));
      }
      if(!panelList.get(i).getTaxonomyDescription().isEmpty()) {
        descriptions.push(asTextDto(panelList.get(i).getTaxonomyDescription(), availableLocales.get(i)));
      }
    }
