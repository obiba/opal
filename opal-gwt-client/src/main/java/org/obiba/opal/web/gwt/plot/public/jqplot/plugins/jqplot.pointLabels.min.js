...
(function (c) {
    ...
    c.jqplot.PointLabels.prototype.setLabels = function () {
        ...
        if (f.seriesLabelIndex != null) {
            ...
            if (this.renderer.constuctor == c.jqplot.BarRenderer && this.barDirection == "horizontal") {
                h = 0
            } else {
    ...
    c.jqplot.PointLabels.draw = function (t, h) {
        var r = this.plugins.pointLabels;
        r.setLabels.call(this);
        for (var s = 0; s < r._elems.length; s++) {
            r._elems[s].remove()
        }
        if (r.show) {
            var o = "_" + this._stackAxis + "axis";
            if (!r.formatString) {
                r.formatString = this[o]._ticks[0].formatString;
                r.formatter = this[o]._ticks[0].formatter
            }
            var z = this._plotData;
            var w = this._xaxis;
            var n = this._yaxis;
            for (var s = r._labels.length - 1; s >= 0; s--) {
                var m = r._labels[s];
                if (r.hideZeros && parseInt(r._labels[s], 10) == 0) {
                    m = ""
                }
                if (m != null) {
                    m = r.formatter(r.formatString, m)
                }
                var v = c('<div class="jqplot-point-label jqplot-series-' + this.index + " jqplot-point-" + s + '" style="position:absolute"></div>');
                v.insertAfter(t.canvas);
                r._elems.push(v);
                if (r.escapeHTML) {
                    v.text(m)
                } else {
                ...
                var f = r.location;
                if (this.waterfall && parseInt(m, 10) < 0) {
                    f = b[d[f]]
                }
                var l = w.u2p(z[s][0]) + r.xOffset(v, f);
                var g = n.u2p(z[s][1]) + r.yOffset(v, f);
                ...
                var j = l + c(v).width();
                var q = g + c(v).height();
                var y = r.edgeTolerance;
                var e = c(t.canvas).position().left;
                var u = c(t.canvas).position().top;
                var x = t.canvas.width + e;
                var k = t.canvas.height + u;
                if (l - y < e || g - y < u || j + y > x || q + y > k) {
                    c(v).detach()
                }
            }
        }
