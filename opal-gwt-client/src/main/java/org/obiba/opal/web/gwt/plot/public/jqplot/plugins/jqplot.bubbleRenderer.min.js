...
(function (f) {
    ...
    f.jqplot.BubbleRenderer.prototype.init = function (v, s) {
        ...
        this.labels = [];
        this.bubbleCanvases = [];
        if (v.highlightMouseDown && v.highlightMouseOver == null) {
            v.highlightMouseOver = false
        }
        f.extend(true, this, v);
        ...
        for (var q = 0; q < this.data.length; q++) {
            var o = null;
            var u = this.data[q];
            this.maxRadius = Math.max(this.maxRadius, u[2]);
            if (u[3]) {
                if (typeof(u[3]) == "object") {
                    o = u[3]["color"]
                }
            }
            if (o == null) {
                if (this.seriesColors[q] != null) {
                    o = this.seriesColors[q]
                }
            }
            if (o && this.bubbleAlpha < 1) {
                comps = f.jqplot.getColorComponents(o);
                o = "rgba(" + comps[0] + ", " + comps[1] + ", " + comps[2] + ", " + this.bubbleAlpha + ")"
            }
            if (o) {
                this.seriesColors[q] = o
            }
        }
        ...
        if (this.highlightColors.length == 0) {
            for (var q = 0; q < this.seriesColors.length; q++) {
                var n = f.jqplot.getColorComponents(this.seriesColors[q]);
                var t = [n[0], n[1], n[2]];
                var r = t[0] + t[1] + t[2];
                for (var p = 0; p < 3; p++) {
                    t[p] = (r > 570) ? t[p] * 0.8 : t[p] + 0.3 * (255 - t[p]);
                    t[p] = parseInt(t[p], 10)
                }
                this.highlightColors.push("rgba(" + t[0] + "," + t[1] + "," + t[2] + ", " + this.highlightAlpha + ")")
            }
        }
        ...
        s.eventListenerHooks.addOnce("jqplotMouseMove", a);
        s.eventListenerHooks.addOnce("jqplotMouseDown", b);
        s.eventListenerHooks.addOnce("jqplotMouseUp", k);
        s.eventListenerHooks.addOnce("jqplotClick", g);
        s.eventListenerHooks.addOnce("jqplotRightClick", l);
        s.postDrawHooks.addOnce(h)
    };
    ...
    f.jqplot.BubbleRenderer.prototype.draw = function (C, I, n) {
        if (this.plugins.pointLabels) {
            this.plugins.pointLabels.show = false
        }
        var z = (n != undefined) ? n : {};
        var q = (z.shadow != undefined) ? z.shadow : this.shadow;
        this.canvas._elem.empty();
        for (var F = 0; F < this.radii.length; F++) {
            var B = this.radii[F][0];
            var v = null;
            var E = null;
            var m = tel = null;
            var H = this.data[B];
            var I = this.gridData[B];
            if (H[3]) {
                if (typeof(H[3]) == "object") {
                    v = H[3]["label"]
                } else {
            ...
            E = this.colorGenerator.get(B);
            var D = I[2];
            var p, J;
            if (this.shadow) {
                p = (0.7 + I[2] / 40).toFixed(1);
                J = 1 + Math.ceil(I[2] / 15);
                D += p * J
            }
            this.bubbleCanvases[B] = new f.jqplot.BubbleCanvas();
            this.canvas._elem.append(this.bubbleCanvases[B].createElement(I[0], I[1], D));
            this.bubbleCanvases[B].setContext();
            var C = this.bubbleCanvases[B]._ctx;
            var s = C.canvas.width / 2;
            var r = C.canvas.height / 2;
            if (this.shadow) {
                this.renderer.shadowRenderer.draw(C, [s, r, I[2], 0, 2 * Math.PI], {offset: p, depth: J})
            }
            this.bubbleCanvases[B].draw(I[2], E, this.bubbleGradients, this.shadowAngle / 180 * Math.PI);
            if (v) {
                tel = f('<div style="position:absolute;" class="jqplot-bubble-label"></div>');
                if (this.escapeHtml) {
                    tel.text(v)
                } else {
                    tel.html(v)
                ...
                var G = f(tel).outerHeight();
                var u = f(tel).outerWidth();
                var A = I[1] - 0.5 * G;
                var o = I[0] - 0.5 * u;
                tel.css({top: A, left: o});
                this.labels[B] = f(tel)
            }
        }
    };
    ...
    f.jqplot.BubbleAxisRenderer.prototype.init = function (z) {
        f.extend(true, this, z);
        var x = this._dataBounds;
        var t = minpidx = maxsids = maxpidx = maxr = minr = minMaxRadius = maxMaxRadius = maxMult = minMult = 0;
        for (var o = 0; o < this._series.length; o++) {
            var y = this._series[o];
            var u = y._plotData;
            for (var n = 0; n < u.length; n++) {
                if (this.name == "xaxis" || this.name == "x2axis") {
                    if (u[n][0] < x.min || x.min == null) {
                        x.min = u[n][0];
                        t = o;
                        minpidx = n;
                        minr = u[n][2];
                        minMaxRadius = y.maxRadius;
                        minMult = y.autoscaleMultiplier
                    }
                    if (u[n][0] > x.max || x.max == null) {
                        x.max = u[n][0];
                        maxsidx = o;
                        maxpidx = n;
                        maxr = u[n][2];
                        maxMaxRadius = y.maxRadius;
                        maxMult = y.autoscaleMultiplier
                    }
                } else {
                    if (u[n][1] < x.min || x.min == null) {
                        x.min = u[n][1];
                        t = o;
                        minpidx = n;
                        minr = u[n][2];
        ...
        var r = minr / minMaxRadius;
        var m = maxr / maxMaxRadius;
        var w = x.max - x.min;
        var q = Math.min(this._plotDimensions.width, this._plotDimensions.height);
        var v = r * minMult / 3 * w;
        var p = m * maxMult / 3 * w;
        x.max += p;
        x.min -= v
    ...
    function a(q, p, t, s, r) {
        if (s) {
            ...
            if (r.series[o[0]].highlightMouseOver && !(o[0] == r.plugins.bubbleRenderer.highlightedSeriesIndex && o[1] == r.series[o[0]]._highlightedPoint)) {
    ...
    function b(p, o, s, r, q) {
        if (r) {
            ...
            if (q.series[n[0]].highlightMouseDown && !(n[0] == q.plugins.bubbleRenderer.highlightedSeriesIndex && n[1] == q.series[n[0]]._highlightedPoint)) {
                var m = jQuery.Event("jqplotDataHighlight");
                m.pageX = p.pageX;
                m.pageY = p.pageY;
    ...
    function l(q, p, t, s, r) {
        if (s) {
            var o = [s.seriesIndex, s.pointIndex, s.data];
            var m = r.plugins.bubbleRenderer.highlightedSeriesIndex;
